...
func clientTasks(client ClientTasksClient, tmpl Template) Handler {
	return func(app WorkflowVars, w http.ResponseWriter, r *http.Request) error {
		...
		case http.MethodGet:
			...
			fmt.Println("just before template")
			fmt.Println(vars.App.EnvironmentVars)
			fmt.Println(vars.App.Path)
			fmt.Println(vars.App.MyDetails)
			fmt.Println(vars.App.SelectedTeam)
